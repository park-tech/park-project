...
struct	Movefilter  cell_temp[8];          //单体电池温度
struct	Movefilter  cell_vol_b0[8];        //单体电池电压——正
struct	Movefilter  cell_vol_b1[8];        //单体电池电压——负
...
{	
    ...
    if ((u32_timer_tick % TIMEBASE_10MS ) == 0)    timer_flag.flag_10ms = 1;
    if ((u32_timer_tick % TIMEBASE_20MS) == 0)     timer_flag.flag_20ms = 1; 
    if ((u32_timer_tick % TIMEBASE_50MS) == 0)     timer_flag.flag_50ms = 1;
    if ((u32_timer_tick % TIMEBASE_100MS) == 0)    timer_flag.flag_100ms = 1;
    if ((u32_timer_tick % TIMEBASE_200MS) == 0)    timer_flag.flag_200ms = 1;
    if ((u32_timer_tick % TIMEBASE_500MS) == 0)    timer_flag.flag_500ms = 1;
    if ((u32_timer_tick % TIMEBASE_1S) == 0)       timer_flag.flag_1s = 1;
    if ((u32_timer_tick % TIMEBASE_2S) == 0)       timer_flag.flag_2s = 1;
    if ((u32_timer_tick % TIMEBASE_5S) == 0)       timer_flag.flag_5s = 1;
    if ((u32_timer_tick % TIMEBASE_10S) == 0)      timer_flag.flag_10s = 1;
    if ((u32_timer_tick % TIMEBASE_20S) == 0)      timer_flag.flag_20s = 1;
    if ((u32_timer_tick % TIMEBASE_30S) == 0)      timer_flag.flag_30s = 1;
    if ((u32_timer_tick % TIMEBASE_1MIN) == 0)     timer_flag.flag_1min = 1;
    if ((u32_timer_tick % TIMEBASE_2MIN) == 0)     timer_flag.flag_2min = 1;
    if ((u32_timer_tick % TIMEBASE_5MIN) == 0)     timer_flag.flag_5min = 1;
    if ((u32_timer_tick % TIMEBASE_10MIN) == 0)    timer_flag.flag_5min = 1;

    if (u32_timer_tick > TIMEBASE_10MIN)            u32_timer_tick = 0;
}
/*****************************************************************************************
** 函数名称：
** 函数描述：ADC采样,并发送消息队列
...
static void  cell_adc_sample(void)
{
			...
			if(cell_vol_b1[ADCchannelindex].u16_avg_value>cell_vol_b0[ADCchannelindex].u16_avg_value)
			{
			...
			if(ADCchannelindex==st_product_info.u8_bat_num)
			{
				ADCchannelindex=0;
			}
...
static void bat_Qnow_updata(void)
{
	...
	Q_unit = ((double)(fl_bat_all_I + fl_bat_lastall_I)/2) * Integral_time
		       * fmod_bat_effect (st_bat_data.fl_bat_dischI, st_bat_data.fl_bat_max_temp) ; //本次积分的容量
...
static float fmod_bat_effect (float bat_i, float tempb)
{
	...
	if(bat_IC > 0.2f  && bat_IC <= 0.5f  )  current_k = 100 / (-46.67f * bat_IC + 109.33f); 
	if(bat_IC > 0.5f  && bat_IC <= 1  )  current_k = 100 / (-21 * bat_IC + 96.5f);
	if(bat_IC > 1  )	current_k = 100 / (-11 * bat_IC + 86.5f);
	
	if(tempb <= -40)                    temp_k =  100 / 37.5f;
    if((tempb > -40) && (tempb <= -15)) temp_k =  100 / (1.3f*tempb + 89.5f);    //与25度时100容量为100时对应的比值
    if((tempb > -15) && (tempb <= 0))   temp_k =  100 / (tempb + 85);
	if((tempb > 0) && (tempb <= 15))    temp_k =  100 / (0.66667f * tempb + 85);
	if((tempb > 15) && (tempb <= 30))   temp_k =  100 / (0.5333f * tempb + 87);
	if((tempb > 30) && (tempb <= 45))   temp_k =  100 / (0.6f * tempb + 85);
	if(tempb > 45)                      temp_k =  100 / 109;
