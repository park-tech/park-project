...
static struct ftl_flash_device  ftl_flash_device;

static void flash_lock(struct ftl_flash_device * flash_device)
{
    rt_mutex_take(&flash_device->lock, RT_WAITING_FOREVER);
}

static void flash_unlock(struct ftl_flash_device * flash_device)
{
    rt_mutex_release(&flash_device->lock);
}

static rt_err_t ftl_flash_init(rt_device_t dev)
{
    return RT_EOK;
}

static rt_err_t ftl_flash_open(rt_device_t dev, rt_uint16_t oflag)
{
    return RT_EOK;
}

static rt_err_t ftl_flash_close(rt_device_t dev)
{
    return RT_EOK;
}

static rt_err_t ftl_flash_control(rt_device_t dev, int cmd, void *args)
{
    ...
    if (cmd == RT_DEVICE_CTRL_BLK_GETGEOME)
    {
        ...
        geometry->bytes_per_sector = ftl_flash_device.geometry.bytes_per_sector;
        geometry->sector_count = ftl_flash_device.geometry.sector_count;
        geometry->block_size = ftl_flash_device.geometry.block_size;
    }

    return RT_EOK;
}

static rt_size_t ftl_flash_read(rt_device_t dev,
                                   rt_off_t pos,
                                   void* buffer,
                                   rt_size_t size)
{
    flash_lock((struct ftl_flash_device *)dev);
	// rt_kprintf("%s pos = %d size = %d buffersize = %d\r\n",__func__,pos,size,sizeof(buffer));
	FTL_ReadSectors((rt_uint8_t *)buffer,pos,NAND_ECC_SECTOR_SIZE,size);
    flash_unlock((struct ftl_flash_device *)dev);
...
static rt_size_t ftl_flash_write(rt_device_t dev,
                                    rt_off_t pos,
                                    const void* buffer,
                                    rt_size_t size)
{
    flash_lock((struct ftl_flash_device *)dev);
	// rt_kprintf("%s pos = %d size = %d buffersize = %d\r\n",__func__,pos,size,sizeof(buffer));
	FTL_WriteSectors((rt_uint8_t *)buffer,pos,NAND_ECC_SECTOR_SIZE,size);
    flash_unlock((struct ftl_flash_device *)dev);
...
{
	if (rt_mutex_init(&ftl_flash_device.lock, "ftl", RT_IPC_FLAG_FIFO) != RT_EOK)
    {
        rt_kprintf("init ftl lock mutex failed\n");
        return -RT_ENOSYS;
    }
    rt_uint8_t temp;
    if(rt_hw_mtd_nand_init()){
		return 1;
	}									//初始化NAND FLASH
	if(nand_dev.lut){
		rt_free(nand_dev.lut);
	}
	nand_dev.lut = (rt_uint16_t *)rt_malloc(2*nand_dev.block_totalnum);  //给LUT表申请内存
	memset(nand_dev.lut,0,nand_dev.block_totalnum*2);			//全部清理
    if(!nand_dev.lut){
		return RT_ERROR;				//内存申请失败
	} 
    temp = FTL_CreateLUT(1);
    if(temp){   
        rt_kprintf("format nand flash...\r\n");
	...
	ftl_flash_device.geometry.bytes_per_sector = 512;
	ftl_flash_device.geometry.sector_count = nand_dev.valid_blocknum*nand_dev.block_pagenum*nand_dev.page_mainsize/512;
	ftl_flash_device.geometry.block_size = nand_dev.page_mainsize;

	/* register device */
    ftl_flash_device.flash_device.type    = RT_Device_Class_Block;
    ftl_flash_device.flash_device.init    = ftl_flash_init;
    ftl_flash_device.flash_device.open    = ftl_flash_open;
    ftl_flash_device.flash_device.close   = ftl_flash_close;
    ftl_flash_device.flash_device.read 	  = ftl_flash_read;
    ftl_flash_device.flash_device.write   = ftl_flash_write;
    ftl_flash_device.flash_device.control = ftl_flash_control;
    /* no private */
    ftl_flash_device.flash_device.user_data = RT_NULL;

	rt_device_register(&ftl_flash_device.flash_device, "w29n02g",
                       RT_DEVICE_FLAG_RDWR | RT_DEVICE_FLAG_STANDALONE);
	return RT_EOK;
} 
INIT_DEVICE_EXPORT(FTL_Init);
//标记某一个块为坏块
//blocknum:块编号,范围:0~(block_totalnum-1)
void FTL_BadBlockMark(rt_uint32_t blocknum)
{
    rt_uint32_t temp=0xAAAAAAAA;//坏块标记mark,任意值都OK,只要不是0XFF.这里写前4个字节,方便FTL_FindUnusedBlock函数检查坏块.(不检查备份区,以提高速度)
    NAND_WriteSpare(blocknum*nand_dev.block_pagenum,0,(rt_uint8_t*)&temp,4);	//在第一个page的spare区,第一个字节做坏块标记(前4个字节都写)
    NAND_WriteSpare(blocknum*nand_dev.block_pagenum+1,0,(rt_uint8_t*)&temp,4);	//在第二个page的spare区,第一个字节做坏块标记(备份用,前4个字节都写)
} 
//检查某一块是否是坏块
//blocknum:块编号,范围:0~(block_totalnum-1)
//返回值:0,好块
...
{
    rt_uint8_t Usedflag=0xCC;
    rt_uint8_t temp=0;
    temp=NAND_WriteSpare(blocknum*nand_dev.block_pagenum,1,(rt_uint8_t*)&Usedflag,1);//写入块已经被使用标志
    return temp;
}   
...
{
    rt_uint32_t temp=0;
    rt_uint32_t blocknum=0; 
	for(blocknum=sblock+1;blocknum>0;blocknum--)
    {
        if(((blocknum-1)%2)==flag)//奇偶合格,才检测
		{
		    NAND_ReadSpare((blocknum-1)*nand_dev.block_pagenum,0,(rt_uint8_t*)&temp,4);//读块是否被使用标记
...
{
	static rt_uint32_t curblock=0XFFFFFFFF;
	rt_uint32_t unusedblock=0;  
...
{
    rt_uint16_t i=0,temp=0,wrlen;
    rt_uint32_t source_block=0,pageoffset=0;
    rt_uint32_t unusedblock=0; 
...
rt_uint8_t FTL_WriteSectors(rt_uint8_t *pBuffer,rt_uint32_t SectorNo,rt_uint16_t SectorSize,rt_uint32_t SectorCount)
{
	...
	rt_uint16_t temp;
    rt_uint32_t i=0;
	rt_uint16_t wsecs;		//写页大小
	rt_uint32_t wlen;		//写入长度
    rt_uint32_t LBNNo;      //逻辑块号
    rt_uint32_t PBNNo;      //物理块号
    rt_uint32_t PhyPageNo;  //物理页号
    rt_uint32_t PageOffset; //页内偏移地址
    rt_uint32_t BlockOffset;//块内偏移地址
	rt_uint32_t markdpbn=0xFFFFFFFF;		//标记了的物理块编号  
	for(i=0;i<SectorCount;i++)
    {
        ...
        rt_kprintf("PhyPageNo = 0x%08x PageOffset = %d wlen = %d\r\n",PhyPageNo,PageOffset,wlen);
		//读出写入大小的内容判断是否全为0XFF
        ...
        {
			temp=((rt_uint32_t)nand_dev.block_pagenum*nand_dev.page_mainsize-BlockOffset)/SectorSize;//计算整个block还剩下多少个SECTOR可以写入
 			wsecs=SectorCount-i;				//还剩多少个sector要写
			if(wsecs>=temp)wsecs=temp;			//大于可连续写入的sector数,则写入temp个扇区 
			wlen=wsecs*SectorSize;				//每次写wsecs个sector   
...
rt_uint8_t FTL_BlockCompare(rt_uint32_t blockx,rt_uint32_t cmpval)
{
	...
	for(i=0;i<3;i++)//允许3次机会
	{
		...
		if(j==nand_dev.block_pagenum)return RT_EOK;		//检查合格,直接退出
		res=NAND_EraseBlock(blockx);
		if(res)rt_kprintf("error erase block:%d\r\n",i);
		else
		{ 
	...
	rt_kprintf("bad block checked:%d\r\n",blockx);
...
rt_uint32_t FTL_SearchBadBlock(void)
{
	...
	rt_uint8_t res;
	rt_uint32_t i,j; 
	rt_uint32_t goodblock=0;
	blktbl = (rt_uint8_t *)rt_malloc(nand_dev.block_totalnum*sizeof(rt_uint8_t*));	//申请block坏块表内存,对应项:0,好块;1,坏块;
	NAND_EraseChip(); 						//全片擦除
...
rt_uint8_t FTL_Format(void)
{
    ...
    for(i=0;i<nand_dev.block_totalnum;i++)		//在好块中标记上逻辑块信息
    {
        ...
        if(temp==0)                  			//好块
        { 
            NAND_WriteSpare(i*nand_dev.block_pagenum,2,(rt_uint8_t*)&n,2);//写入逻辑块编号
            n++;								//逻辑块编号加1
